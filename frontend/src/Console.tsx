...
const workFormats = [["unknown","Не вказано"],["onsite","На місці / в офісі"],["remote","Віддалено"],["hybrid","Гібрид"],["any","Будь-який"]];
const employmentTypes = [["unknown","Не вказано"],["full_or_part_time","Повна або часткова"],["part_time","Часткова"],["full_time","Повна"]];
...
export function ConsolePersons(){
  ...
  const [items,setItems]=useState<PersonListItem[]>([]),[error,setError]=useState(""),[loading,setLoading]=useState(true),[query,setQuery]=useState(""),[stageFilter,setStageFilter]=useState(""),[workFormatFilter,setWorkFormatFilter]=useState(""),[employmentTypeFilter,setEmploymentTypeFilter]=useState(""),[dateSort,setDateSort]=useState("newest"),[quickFilter,setQuickFilter]=useState("");
  const [stageBusy,setStageBusy]=useState<string|null>(null),[closingPerson,setClosingPerson]=useState<PersonListItem|null>(null),[closureReason,setClosureReason]=useState(""),[closureNote,setClosureNote]=useState(""),[closureError,setClosureError]=useState("");
  ...
  const filtered=items.filter(p=>(!stageFilter||(p.workflow_stage||"new_request")===stageFilter)&&(!workFormatFilter||(p.work_format||"unknown")===workFormatFilter)&&(!employmentTypeFilter||(p.employment_type||"unknown")===employmentTypeFilter)&&filterMatches(p)&&`${p.name} ${p.phone??""} ${p.email??""} ${p.city??""} ${p.responsible?.full_name??""}`.toLowerCase().includes(query.toLowerCase())).sort((a,b)=>{const aTime=Date.parse(a.created_at||a.updated_at||""),bTime=Date.parse(b.created_at||b.updated_at||"");if(!Number.isFinite(aTime))return 1;if(!Number.isFinite(bTime))return -1;return dateSort==="oldest"?aTime-bTime:bTime-aTime});
  ...
  const cancelClosing=()=>{if(stageBusy)return;setClosingPerson(null);setClosureReason("");setClosureNote("");setClosureError("")};
  const saveStage=async(person:PersonListItem,workflowStage:string,reason?:string,note?:string)=>{if(stageBusy||workflowStage===(person.workflow_stage||"new_request"))return;setStageBusy(person.id);setError("");setClosureError("");try{await adminRequest<Person>(`/admin/persons/${person.id}/workflow`,{method:"PATCH",body:JSON.stringify({workflow_stage:workflowStage,...(workflowStage==="closed"?{closure_reason:reason,closure_note:note?.trim()||null}:{})})});const stageUk=workflowStages.find(([value])=>value===workflowStage)?.[1]||workflowStage;const reasonUk=workflowStage==="closed"?closureReasons.find(([value])=>value===reason)?.[1]||null:null;setItems(current=>current.map(row=>row.id===person.id?{...row,workflow_stage:workflowStage,workflow_stage_uk:stageUk,workflow_closure_reason_uk:reasonUk,needs_contact:workflowStage==="needs_contact"?true:workflowStage==="new_request"?row.needs_contact:false}:row));if(workflowStage==="closed"){setClosingPerson(null);setClosureReason("");setClosureNote("")}}catch(e){const message=errText(e);if(workflowStage==="closed")setClosureError(message);else setError(message)}finally{setStageBusy(null)}};
  const chooseStage=(person:PersonListItem,value:string)=>{if(value==="closed"&&value!==(person.workflow_stage||"new_request")){setClosingPerson(person);setClosureReason("");setClosureNote("");setClosureError("");return}void saveStage(person,value)};
  const confirmClosing=(e:FormEvent)=>{e.preventDefault();if(!closingPerson||stageBusy)return;if(!closureReason){setClosureError("Оберіть причину закриття клієнта");return}if(closureReason==="other"&&!closureNote.trim()){setClosureError("Уточніть причину закриття");return}void saveStage(closingPerson,"closed",closureReason,closureNote)};
  return <section><div className="page-title"><div><span className="console-kicker">РОБОТА З ЛЮДЬМИ</span><h1>{role==="manager"?"Мої клієнти":"Усі клієнти"}</h1><p>{role==="manager"?"Створені вами та доступні вам профілі.":"Єдина база клієнтів вашої команди."}</p></div><Link className="button" to="/admin/persons/new">+ Додати клієнта</Link></div><div className="console-metrics console-person-metrics">{[["Доступних клієнтів",items.length],["Кейсів",items.filter(p=>p.status==="case").length],["Активних",items.filter(p=>p.status==="active").length],["Чернеток",items.filter(p=>p.status==="draft").length]].map(([label,value])=><article key={label}><span>{label}</span><strong>{loading?"—":value}</strong></article>)}</div><div className="console-list-panel"><div className="workflow-quick-filters" aria-label="Швидкі фільтри">{quickFilters.map(([value,label,count])=><button type="button" key={value} className={quickFilter===value?"active":""} aria-pressed={quickFilter===value} onClick={()=>setQuickFilter(quickFilter===value?"":value)}><span>{label}</span><b>{loading?"—":count}</b></button>)}</div><div className="console-filters"><input aria-label="Пошук клієнтів" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Ім’я, телефон, email, місто або консультант"/><select aria-label="Статус" value={stageFilter} onChange={e=>setStageFilter(e.target.value)}><option value="">Усі статуси</option>{workflowStages.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select><select aria-label="Формат роботи" value={workFormatFilter} onChange={e=>setWorkFormatFilter(e.target.value)}><option value="">Усі формати роботи</option>{workFormats.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select><select aria-label="Тип зайнятості" value={employmentTypeFilter} onChange={e=>setEmploymentTypeFilter(e.target.value)}><option value="">Усі типи зайнятості</option>{employmentTypes.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select><select aria-label="Сортування за датою додавання" value={dateSort} onChange={e=>setDateSort(e.target.value)}><option value="newest">Спочатку нові</option><option value="oldest">Спочатку старі</option></select><button className="button secondary" onClick={load} disabled={loading}>Оновити</button></div><Notice error={error}/><div className="table-wrap"><table><thead><tr><th>Клієнт</th><th>Контакти</th><th>Місто</th><th>Супровід</th><th>Статус</th><th/></tr></thead><tbody>{!loading&&filtered.map(p=><tr key={p.id}><td><Link className="console-person-link" to={`/admin/persons/${p.id}`}><span className="console-avatar">{p.name?.[0]||"?"}</span><b>{p.name||"Без імені"}</b></Link></td><td>{p.phone||"—"}<small>{p.email}</small></td><td>{p.city||"—"}</td><td><div className="workflow-table-cell"><b>{p.responsible?.full_name||p.responsible?.email||"Без відповідального"}</b><span className={`status ${p.status}`}>{p.status_uk}</span>{p.needs_contact?<small className="needs-contact-label">Потрібно зв’язатися</small>:p.next_action_at?<small>Наступна дія: {new Date(p.next_action_at).toLocaleString("uk-UA",{day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit"})}</small>:<small>Наступної дії немає</small>}</div></td><td><select className="workflow-stage-select" aria-label={`Статус клієнта ${p.name||"без імені"}`} value={p.workflow_stage||"new_request"} disabled={stageBusy===p.id} onChange={e=>chooseStage(p,e.target.value)}>{workflowStages.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></td><td><Link className="button secondary small" to={`/admin/persons/${p.id}`}>Відкрити →</Link></td></tr>)}</tbody></table>{loading?<p className="loading">Завантажуємо клієнтів…</p>:!filtered.length&&<div className="console-empty"><h2>{query||stageFilter||workFormatFilter||employmentTypeFilter||quickFilter?"Нічого не знайдено":"Тут будуть ваші клієнти"}</h2><p>{query||stageFilter||workFormatFilter||employmentTypeFilter||quickFilter?"Змініть пошук або фільтр.":"Додайте першого клієнта, щоб почати роботу."}</p></div>}</div><div className="console-list-footer">Показано {filtered.length} із {items.length}</div></div>{closingPerson&&<div className="console-modal-backdrop" onClick={e=>{if(e.target===e.currentTarget)cancelClosing()}}><section className="console-modal list-close-modal" role="dialog" aria-modal="true" aria-labelledby="close-person-title"><div className="console-modal-head"><div><span className="console-kicker">ЗАКРИТТЯ КЛІЄНТА</span><h2 id="close-person-title">Вкажіть причину</h2><p>{closingPerson.name}</p></div><button type="button" className="console-modal-close" onClick={cancelClosing} aria-label="Закрити">×</button></div><form className="form list-close-form" onSubmit={confirmClosing}><label><span>Причина закриття *</span><select required autoFocus value={closureReason} disabled={Boolean(stageBusy)} onChange={e=>setClosureReason(e.target.value)}><option value="">Оберіть причину</option>{closureReasons.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label>{closureReason==="other"&&<label><span>Уточніть причину *</span><textarea required maxLength={500} value={closureNote} disabled={Boolean(stageBusy)} placeholder="Коротко вкажіть причину" onChange={e=>setClosureNote(e.target.value)}/></label>}<Notice error={closureError}/><div className="console-form-actions"><button type="button" className="button secondary" disabled={Boolean(stageBusy)} onClick={cancelClosing}>Скасувати</button><button className="button" disabled={Boolean(stageBusy)}>{stageBusy?"Зберігаємо…":"Закрити клієнта"}</button></div></form></section></div>}</section>
}
...
function MobilityEditor({person,onSaved}:{person:Person;onSaved:(p:Person)=>void}){
  ...
  const save=async(e:FormEvent)=>{e.preventDefault();setBusy(true);setError("");try{const keys=["has_driver_license","driver_license_categories","has_car","willing_to_relocate","work_format","employment_type","work_geography"];onSaved(await adminRequest<Person>(`/admin/persons/${person.id}`,{method:"PATCH",body:JSON.stringify(Object.fromEntries(keys.filter(k=>k in values).map(k=>[k,values[k]])))}))}catch(e){setError(errText(e))}finally{setBusy(false)}};
  return <form className="panel form console-editor" onSubmit={save}><h2>Формат роботи та мобільність</h2><div className="form-grid">{[["has_driver_license","Посвідчення водія"],["has_car","Власне авто"],["willing_to_relocate","Готовність до переїзду"]].map(([key,label])=><label key={key}><span>{label}</span><select value={String(values[key]||"unknown")} onChange={e=>setValues({...values,[key]:e.target.value})}><option value="unknown">Не вказано</option><option value="yes">Так</option><option value="no">Ні</option></select></label>)}<label><span>Категорії водійського посвідчення</span><input value={String(values.driver_license_categories||"")} onChange={e=>setValues({...values,driver_license_categories:e.target.value})}/></label><label><span>Формат роботи</span><select value={String(values.work_format||"unknown")} onChange={e=>setValues({...values,work_format:e.target.value})}>{workFormats.map(([key,label])=><option key={key} value={key}>{label}</option>)}</select></label><label><span>Тип зайнятості</span><select value={String(values.employment_type||"unknown")} onChange={e=>setValues({...values,employment_type:e.target.value})}>{employmentTypes.map(([key,label])=><option key={key} value={key}>{label}</option>)}</select></label></div><Notice error={error}/><div className="console-form-actions"><button className="button" disabled={busy}>{busy?"Зберігаємо…":"Зберегти зміни"}</button></div></form>
}
...
function QuickWorkflowCard({person,role,staffId,onSaved}:{person:Person;role:string|null;staffId:number|null;onSaved:(person:Person)=>void}){
  ...
  const [busy,setBusy]=useState(false),[statusBusy,setStatusBusy]=useState(false),[error,setError]=useState("");
  ...
  const changePersonStatus=async(status:string)=>{if(role!=="super_admin"||statusBusy||status===person.core.status)return;setStatusBusy(true);setError("");try{onSaved(await adminRequest<Person>(`/admin/persons/${person.id}`,{method:"PATCH",body:JSON.stringify({status})}))}catch(e){setError(errText(e))}finally{setStatusBusy(false)}};
  return <form className="profile-section-surface profile-workflow-quick-card" onSubmit={save}>
    <div className="profile-card-heading"><span className="profile-step-number">01</span><div><h2>Хто і коли діє далі</h2><p>Відповідальний консультант, наступний етап і запланована дата</p></div></div>
    <div className="support-owner-grid support-owner-single"><label><span>Відповідальний консультант</span>{canManage?<select value={responsibleId} disabled={busy} onChange={e=>setResponsibleId(e.target.value)}><option value="">Не призначено</option>{consultantOptions.map(member=><option key={member.id} value={member.id} disabled={!member.is_active}>{member.full_name||member.email} · {roles[member.role]||member.role}{!member.is_active?" (вимкнений)":""}</option>)}</select>:<div className="responsible-readonly"><b>{current.responsible?.full_name||current.responsible?.email||"Не призначено"}</b>{managerCanTake&&<button type="button" className="button secondary small" onClick={()=>setResponsibleId(String(staffId))}>Взяти на себе</button>}{responsibleId===String(staffId)&&current.responsible?.id!==staffId&&<small>Буде призначено після збереження</small>}</div>}</label></div>
    <div className="quick-card-status-row"><div><span>Тип картки</span><small>КЕЙС, чернетка, активний клієнт або архів</small></div>{role==="super_admin"?<label className={`profile-status-control ${person.core.status||"case"}`}><span className="sr-only">Тип картки</span><select aria-label="Тип картки" value={person.core.status||"case"} disabled={statusBusy} onChange={e=>void changePersonStatus(e.target.value)}>{personStatuses.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label>:<span className={"status "+(person.core.status||"case")}>{person.core.status_uk||personStatuses.find(([value])=>value===person.core.status)?.[1]||"КЕЙС"}</span>}</div>
    <div className="next-action-grid"><label><span>Статус</span><select value={workflowStage} disabled={busy} onChange={e=>{setWorkflowStage(e.target.value);if(e.target.value!=="closed"){setClosureReason("");setClosureNote("")}}}>{workflowStages.map(([value,label],index)=><option key={value} value={value}>{index+1}. {label}</option>)}</select></label><label><span>Дата і час</span><input type="datetime-local" value={nextAt} disabled={busy} onChange={e=>setNextAt(e.target.value)}/></label></div>
    {workflowStage==="closed"&&<div className="closure-reason-grid"><label><span>Причина закриття *</span><select required value={closureReason} disabled={busy} onChange={e=>setClosureReason(e.target.value)}><option value="">Оберіть причину</option>{closureReasons.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label>{closureReason==="other"&&<label><span>Уточніть причину *</span><input required maxLength={500} value={closureNote} disabled={busy} placeholder="Коротко вкажіть причину" onChange={e=>setClosureNote(e.target.value)}/></label>}</div>}
  ...
  return <section className="client-profile simplified-profile">
    ...
    <header className="profile-header">
      <div className="profile-identity">
        ...
        <div className="profile-heading"><div className="profile-heading-label"><span className="console-kicker">КАРТКА КЛІЄНТА</span><span className="profile-workflow-stage profile-header-stage">{person.workflow?.stage_uk||"Нова заявка"}{person.workflow?.stage==="closed"&&person.workflow.closure_reason_uk?` · ${person.workflow.closure_reason_uk}`:""}</span></div><h1>{person.core.first_name} {person.core.last_name}</h1><div className="profile-meta"><span><ProfileIcon name="pin"/>{person.core.city||"Місто не вказано"}</span>{person.core.phone&&<a href={"tel:"+person.core.phone.replace(/[^\d+]/g,"")}>{person.core.phone}</a>}<span className="profile-created-at">Додано: {person.created_at?new Date(person.created_at).toLocaleString("uk-UA",{day:"2-digit",month:"long",year:"numeric",hour:"2-digit",minute:"2-digit"}):"дата не зафіксована"}</span></div></div>
      </div>
