...
export function ConsolePersons(){
  ...
  const [items,setItems]=useState<PersonListItem[]>([]),[error,setError]=useState(""),[loading,setLoading]=useState(true),[query,setQuery]=useState(""),[stageFilter,setStageFilter]=useState(""),[dateSort,setDateSort]=useState("newest"),[quickFilter,setQuickFilter]=useState("");
  ...
  const filtered=items.filter(p=>(!stageFilter||(p.workflow_stage||"new_request")===stageFilter)&&filterMatches(p)&&`${p.name} ${p.phone??""} ${p.email??""} ${p.city??""} ${p.responsible?.full_name??""}`.toLowerCase().includes(query.toLowerCase())).sort((a,b)=>{const aTime=Date.parse(a.created_at||a.updated_at||""),bTime=Date.parse(b.created_at||b.updated_at||"");if(!Number.isFinite(aTime))return 1;if(!Number.isFinite(bTime))return -1;return dateSort==="oldest"?aTime-bTime:bTime-aTime});
