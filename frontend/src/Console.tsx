...
export function ConsolePerson(){
  ...
  return <section className="client-profile">
    ...
    <header className="profile-header">
      ...
      <div className="profile-header-action"><button className="button" type="button" onClick={()=>setAnalysisOpen(true)}><ProfileIcon name="spark"/>Проаналізувати</button><small>Додати теги з анкети або CV</small>{person.core.status&&<span className={"status "+person.core.status}>{person.core.status_uk||personStatuses.find(([value])=>value===person.core.status)?.[1]||person.core.status}</span>}</div>
    </header>
