...
export function ConsolePerson(){
  ...
  const tabs=[["core","Основне"],...blocks.map(b=>[b.key,b.key==="skills"&&person.tags.length?`${b.title} (${person.tags.length})`:b.title]),["mobility","Мобільність"],["documents","Документи"],["analysis","Проаналізувати"],...(role!=="manager"?[["access","Доступ"]]:[])];const block=blocks.find(b=>b.key===tab);
