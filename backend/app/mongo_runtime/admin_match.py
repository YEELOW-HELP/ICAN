...
async def _resolve_skill(db, raw: dict) -> tuple[str, dict] | None:
    ...
    skill = await db.mnp_skills.find_one({"$or": [
        {"canonical_name_uk": {"$regex": exact, "$options": "i"}},
        {"canonical_name_en": {"$regex": exact, "$options": "i"}},
    ]})
    if skill:
        return str(skill["_id"]), skill

    alias = await db.mnp_skill_aliases.find_one({"alias": {"$regex": exact, "$options": "i"}})
    if alias:
        skill = await db.mnp_skills.find_one({"_id": str(alias.get("skill_id"))})
        if skill:
            return str(skill["_id"]), skill
    return None
...
async def _career_completeness(db, career: dict) -> dict[str, Any]:
    ...
    training = await db.mnp_career_requirements.count_documents(
        {"career_id": cid, "category": {"$in": ["credential", "education", "other"]}}
    )
    transitions = await db.mnp_career_relations.count_documents(
        {"$or": [{"from_career_id": cid}, {"to_career_id": cid}]}
    )
    basics = bool(
        career.get("canonical_name_uk")
        and career.get("description_short_uk")
        and career.get("career_family_id")
    )
    mode = _mode(career)
...
async def search_skills(
    ...
    _staff=Depends(current_staff),
):
    text = q.strip()
    safe = re.escape(text)
    query = {} if not text else {"$or": [
        {"canonical_name_uk": {"$regex": safe, "$options": "i"}},
        {"canonical_name_en": {"$regex": safe, "$options": "i"}},
    ]}
    direct = [row async for row in db.mnp_skills.find(query).limit(20)]
    ids = {str(row["_id"]) for row in direct}
    if text:
        async for alias in db.mnp_skill_aliases.find({"alias": {"$regex": safe, "$options": "i"}}).limit(20):
    ...
    for skill in direct[:20]:
        aliases = [
            a.get("alias")
            async for a in db.mnp_skill_aliases.find({"skill_id": str(skill["_id"])}).limit(8)
        ]
        result.append({
            "id": str(skill["_id"]),
            "name_uk": skill.get("canonical_name_uk"),
            "name_en": skill.get("canonical_name_en"),
...
async def career_editor(career_id: str, db: Database, _staff=Depends(privileged_staff)):
    ...
    requirements = [
        row async for row in db.mnp_career_requirements.find({"career_id": career_id}).sort("sort_order", 1)
    ]
    transitions = [
        row async for row in db.mnp_career_relations.find(
            {"$or": [{"from_career_id": career_id}, {"to_career_id": career_id}]}
        )
    ]
    return {
        "career": {k: v for k, v in career.items() if k != "_id"}
        | {"id": career_id, "entry_requirements_mode": _mode(career)},
        "skills": [
            {k: v for k, v in row.items() if k != "_id"}
            | {
                "id": str(row["_id"]),
                "skill": (
                    {k: v for k, v in skill_docs.get(str(row.get("skill_id")), {}).items() if k != "_id"}
        ...
        "requirements": [
            {k: v for k, v in row.items() if k != "_id"} | {"id": str(row["_id"])}
    ...
    if "skills" in payload:
        ...
        for index, raw in enumerate(payload.get("skills") or []):
            ...
            if not resolved:
                raise HTTPException(
                    400,
                    f"Unknown skill at row {index + 1}; choose an existing mnp_skills item",
                )
            sid, _skill = resolved
            if sid in seen:
            ...
            normalized.append({
                "_id": str(raw.get("id") or f"{career_id}:{sid}"),
                "career_id": career_id,
                "skill_id": sid,
                ...
                "review_status": raw.get("review_status", "editorial"),
                "sort_order": index,
            })
    ...
    if "requirements" in payload:
        ...
        for index, raw in enumerate(payload.get("requirements") or []):
            normalized_req.append({
                "_id": str(raw.get("id") or f"{career_id}:req:{index}"),
                "career_id": career_id,
                "category": raw.get("category", "other"),
                "hardness": raw.get("hardness", "soft"),
...
async def match_preview(
    ...
    for raw in raw_skills:
        ...
        if not resolved:
            unresolved.append(
                str(raw.get("name") or raw.get("name_uk") or raw.get("id") or "")
            )
            continue
        sid, _skill = resolved
        ...
        person_levels[sid] = PersonLevelInput(
            key=sid,
            proficiency_level=level,
            evidence_strength=float(raw.get("evidence_strength", 1.0)),
        )
    ...
    async for career in db.mnp_careers.find({"status": "active"}):
        ...
        mode = _mode(career)
        req_rows = [
            row async for row in db.mnp_career_skill_requirements.find({"career_id": cid})
        ]
        skill_docs = await _skill_map(db, {str(r.get("skill_id")) for r in req_rows})
        fit_inputs = [
            RequirementInput(
                key=str(r.get("skill_id")),
                importance=r.get("importance", "medium"),
                required_level=r.get("required_level", "working"),
            )
            for r in req_rows
        ]
        fit = compute_weighted_coverage_fit(
            fit_inputs,
            person_levels,
            explanation_prefix="admin_preview_skill_fit",
        )
        ...
        for row in req_rows:
            ...
            label = _skill_label(skill_docs.get(sid))
        ...
        career_requirements = [
            row async for row in db.mnp_career_requirements.find({"career_id": cid}).sort("sort_order", 1)
        ]
        ...
        education = [
            r.get("description") for r in career_requirements if r.get("category") == "education"
        ]
        training = [
            r.get("description")
            for r in career_requirements
            if r.get("category") in ("credential", "other")
        ]
        ...
        results.append({
            ...
            "fit_band": fit.band,
            "confidence": fit.confidence_band,
            "status": readiness,
            "entry_requirements_mode": mode,
            "entry_message": (
                "Можна почати без попереднього досвіду"
                if mode == "open_entry"
                else "Вимоги ще не заповнені"
                if mode == "incomplete"
                else None
            ),
            ...
            "education_requirements": [x for x in education if x],
